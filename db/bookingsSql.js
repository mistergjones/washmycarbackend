module.exports = {
    ...
    GET_WASHER_HISTORICAL_COMPLETIONS: `select bookings.date, bookings.start_time, bookings.booking_status, vehicles.vehicle_type, bookings.washer_completed_proof, bookings.service_fee, owners.suburb from bookings, owners, vehicles where bookings.owner_id = owners.owner_id AND bookings.vehicle_id = vehicles.vehicle_id AND bookings.washer_assigned = $1 AND booking_status = 'F';`,

    // GJ: The below query gets all "bookings" for an individual walker that has completed.
    GET_OWNER_HISTORICAL_COMPLETIONS: `select bookings.date, bookings.start_time, bookings.booking_status, vehicles.vehicle_type, bookings.washer_completed_proof, bookings.service_fee, washers.firtname from bookings, owners, vehicles, washers where bookings.owner_id = owners.owner_id AND bookings.vehicle_id = vehicles.vehicle_id AND bookings.washer_assigned = $1 AND booking_status = 'F';`,

    GET_WASHER_ASSIGNED_JOBS: `select bookings.date, bookings.start_time, bookings.booking_status, vehicles.vehicle_type, bookings.washer_completed_proof, bookings.service_fee from bookings, vehicles where bookings.vehicle_id = vehicles.vehicle_id AND bookings.washer_assigned = $1 AND booking_status = 'A';`,
